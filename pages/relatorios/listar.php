...
if (isset($_POST['pesquisar'])) {
  ...
  if (!empty($_POST['status'])) {
    ...
    $query .= " status_id = " .  $_POST['status'];
  }

  if (!empty($_POST['tipo_convenio'])) {
    if ($tem_where) {
        $query .= " AND ";
    } else {
        $query .= " WHERE ";
    ...
    $query .= " tipo_convenio_id = " .  $_POST['tipo_convenio'];
  }
  ...
  if (!empty($_POST['secretaria'])) {
    ...
    $query .= " secretaria_id = " .  $_POST['secretaria'];
  }
            ...
            <?php if (isset($_POST['pesquisar'])) { ?>
              <div class="box-header">
                ...
                <?php if (isset($dias_vencer)) { ?>
                  <?php if ($dias_vencer != 0 || $dias_vencer === 'mais de 90') { ?>
                    <h3 class="box-title"><b> - A vencer em <?php echo $dias_vencer ?> dias</b></h3>
                  <?php } else { ?>
                    <h3 class="box-title"><b> - Vencidos</b></h3>
                  <?php } ?>
                <?php } else { ?>
            ...
            <div class="box-body">
              <table id="tabela" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <?php if (isset ($_GET['dias-vencer'])) { ?>
                    <?php if ($dias_vencer == 0) { ?>
                      <th style="text-align: center">Vencido há (dias)</th>
                    <?php } else { ?>
                      <th style="text-align: center">Vence em (dias)</th>
                    <?php } ?>
                  <?php } ?>
                  ...
                  <?php
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
                    {
                      ...
                      if (isset ($_GET['dias-vencer'])) {
                        echo "<td align='center'>" . $row['vence_em'] . '</td>';
                      }
                      echo "<td align='center'>" . $row['status'] .'</td>';

                      

                      if ($_SESSION['perfil'] == 'Administrador' || $_SESSION['perfil'] =='Técnico') {
                  ?>
                  <td align='center'><a onclick="return confirm('Deseja realmente excluir?');" href='../../controllers/convenios/excluir.php?id=<?=$id?>' class='btn btn-danger'><i class='fa fa-trash'></i></a>
                  <?php     
